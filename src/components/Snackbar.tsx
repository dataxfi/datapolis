                ...
                <div>
                  <p>
                    {txDetails.txType === "stake" && txDetails.token2.info
                      ? `Stake ${new BigNumber(txDetails.token1.value).dp(5).toString()} OCEAN in ${txDetails.token2.info.symbol}/OCEAN pool`
                      : txDetails.txType === "unstake" && txDetails.shares && txDetails.token2.info
                      ? `Unstake ${new BigNumber(txDetails.shares).dp(5).toString()} OCEAN from ${txDetails.token2.info.symbol}/OCEAN pool`
                      : txDetails.txType === "approve"
                      ? `Unlock ${txDetails.token1.info?.symbol}`
                      : `Trade ${new BigNumber(txDetails.token1.value).dp(5).toString()} ${txDetails.token1.info?.symbol} for ${new BigNumber(txDetails.token2.value)
                          .dp(5)
                          .toString()} ${txDetails.token2.info?.symbol}`}
                  </p>
