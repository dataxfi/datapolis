...
const DesktopNavbar = ({links, text, wallet}: {links: Array<any>, text: Record<any, any>, wallet: string}) => {
    ...
    const { handleConnect, buttonText, chainId, config, accountId } = useContext(GlobalContext)
    ...
    return (
             <div className="md:flex justify-between items-center py-4 border-gray-800 pl-4 pr-2 hidden">
                ...
                <div className="grid grid-flow-col gap-4 items-center">
                    <div className="hidden md:block capitalize border border-type-400 text-type-200 rounded-md px-4 py-1">
                        <h3>{config ? config.getNetwork(String(chainId)) : "Unknown"}</h3>
                    </div>
                    <div className="hidden md:block">
                        <Button text={accountId ? buttonText.substring(0,12) + '...' : buttonText} onClick={() => handleConnect()} classes="hm-btn hm-btn-light" />
                    </div>
                </div>
            </div>
    )
